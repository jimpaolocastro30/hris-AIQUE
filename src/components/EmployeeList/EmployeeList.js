...
const EmployeeList=()=>{
  ...
  const handleLoadMore = () => {
    ...
    const fetchData = () => {
        ...
        Axios.get("http://13.229.91.120:3001/api/read/", options).then((response)=> {
    ...
    //////////////////////////////////////
     async function deleteEmployee (id){
      Axios.delete(`http://13.229.91.120:3001/api/delete/${id}`,{
        id:id
      }).then(res=>{
        Swal.fire({
   ...
   async function EmployeeDeleteHistory(id, dev, devmail){
    Axios.post(`http://13.229.91.120:3001/UserMngmnt/delete/${id}`,{
    devbefore:devmail,
    fullName:email,
    date:hdate,
    method:dev,
